...
module Network.Discord.Gateway where
  ...
  class (DiscordAuth b, DiscordAuth m) => DiscordGate b m | m -> b where
    data VaultKey m a
  ...
  runGateway :: DiscordGate b m => URL -> m () -> b ()
  runGateway (URL (Absolute h) path _) client = liftBaseWith $ \runInBase ->
    runSecureClient (host h) 443 (path ++ "/?v=6") $ \conn ->
      void . runInBase $ run client conn
      
  runGateway _ _ = return ()

  send :: DiscordGate b m => Payload -> m ()
  send payload = do
    conn <- connection
    liftIO . sendTextData conn $ encode payload

  heartbeat :: DiscordGate b m => Int -> m ()
  heartbeat interval = fork . forever $ do
    seqNum <- Heartbeat <$> (get =<< storeFor sequenceKey)
    send seqNum
    liftIO $ threadDelay (interval * 1000)

  setSequence :: DiscordGate b m => Integer -> m ()
  setSequence sq = do
    seqNum <- storeFor sequenceKey
    put seqNum sq

  eventStream :: DiscordGate b m => GatewayState -> m () -> m ()
  ...
  step :: DiscordGate b m => m Payload
  step = do
    conn <- connection
